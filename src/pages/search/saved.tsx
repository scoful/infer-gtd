...
const SavedSearchesPage: NextPage = () => {
  ...
  const {
    ...
    isLoading,
  } = api.search.getSavedSearches.useQuery(undefined, {
    enabled: !!sessionData,
  });
    ...
    (searchParams: any, tags?: any[], projects?: any[]) => {
      ...
      if (
        ...
        const selectedProjects = projects.filter((project) =>
          searchProjectIds.includes(String(project.id)),
        );
      ...
      if (searchParams.updatedAfter || searchParams.updatedBefore) {
        ...
        if (formattedAfter && formattedBefore) {
          timeConditions.push(
            `更新时间: ${formattedAfter} 至 ${formattedBefore}`,
          );
        } else if (formattedAfter) {
      ...
      // 时间跟踪筛选
      if (
        searchParams.hasTimeTracking !== undefined &&
        searchParams.hasTimeTracking !== null
      ) {
        conditions.push({
          type: "tracking",
          label: "时间跟踪",
      ...
      if (
        searchParams.minTimeSpent !== undefined ||
        searchParams.maxTimeSpent !== undefined
      ) {
      ...
      if (
        searchParams.isRecurring !== undefined &&
        searchParams.isRecurring !== null
      ) {
        conditions.push({
          type: "type",
          label: "重复任务",
  ...
  // 编辑搜索条件
  const handleEditSearchConditions = useCallback(
    (search: any) => {
      // 构建查询参数，将搜索条件编码到URL中
      const searchParams = search.searchParams;
      ...
      if (
        ...
        queryParams.set(
          "hasTimeTracking",
          searchParams.hasTimeTracking.toString(),
        );
      }
      if (
        searchParams.minTimeSpent !== undefined &&
        searchParams.minTimeSpent !== null
      ) {
        queryParams.set("minTimeSpent", searchParams.minTimeSpent.toString());
      }
      if (
        searchParams.maxTimeSpent !== undefined &&
        searchParams.maxTimeSpent !== null
      ) {
  ...
  // 保存搜索（编辑模式）
  const handleSaveSearchNew = useCallback(
            ...
            <div className="rounded-lg border border-gray-200 bg-white">
              <div className="border-b border-gray-200 px-6 py-4">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  {searchQuery
                    ? `找到 ${filteredSearches.length} 个匹配的搜索`
                    : `共 ${filteredSearches.length} 个保存的搜索`}
                </p>
              </div>
              <div className="p-6">
